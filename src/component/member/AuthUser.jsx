...
export default function AuthUser({item, onNext}) {
  ...
  const handleReqAuthCode = async() =>{
    try {
      if(refs.emailRef.current.value !== ''){
        const res = await axios.post('http://13.125.244.223:9000/member/authcode'
  ...
  /* 아이디 중복확인 */  
  const handleIdCheck = () =>{
    axios.post('http://13.125.244.223:9000/member/idcheck', {id: formData.id})
         .then((res)=>{
            if(res.data[0].cnt){
  ...
  const handleSubmit = async(e) =>{
    ...
    try {
      if (item === 'id') {
        const { data } = await axios.post('http://13.125.244.223:9000/member/findId', formData);
        if (data[0]?.cnt) {
      ...
      if (item === 'pwd') {
        const { data } = await axios.post('http://13.125.244.223:9000/member/findPwd', formData);
      ...
      if( item === 'userInfo') {
        if(idCheckState){
          const { data } = await axios.post('http://13.125.244.223:9000/member/signup', formData);
